...
Sketchup::require 'circadiem-companion/lib/zip/file_split'

module CircaDiem
  module Zip
  # CircaDiem::Zip::File is modeled after java.util.zip.ZipFile from the Java SDK.
  ...
  #
  # CircaDiem::Zip::File.open('my.zip', create: true) do |zipfile|
  #   zipfile.get_output_stream('first.txt') { |f| f.puts 'Hello from CircaDiem::Zip::File' }
  #   zipfile.mkdir('a_dir')
  # end
  # ```
  #
  ...
  # end
  #
  # CircaDiem::Zip::FileSystem offers an alternative API that emulates ruby's
  ...
  class File
    ...
    def initialize(path_or_io, create: false, buffer: false, **options)
      ...
      options  = DEFAULT_RESTORE_OPTIONS
                 .merge(compression_level: CircaDiem::Zip.default_compression)
                 .merge(options)
    ...
    class << self
      # Similar to ::new. If a block is passed the CircaDiem::Zip::File object is passed
      # to the block and is automatically closed afterwards, just as with
      # ruby's builtin File::open method.
      def open(file_name, create: false, **options)
        zf = CircaDiem::Zip::File.new(file_name, create: create, **options)
      ...
      def open_buffer(io = ::StringIO.new, create: false, **options)
        unless IO_METHODS.map { |method| io.respond_to?(method) }.all? || io.kind_of?(String)
          raise 'CircaDiem::Zip::File.open_buffer expects a String or IO-like argument' \
                "(responds to #{IO_METHODS.join(', ')}). Found: #{io.class}"
        end

        io = ::StringIO.new(io) if io.kind_of?(::String)

        zf = CircaDiem::Zip::File.new(io, create: create, buffer: true, **options)
        return zf unless block_given?
      ...
      def foreach(zip_file_name, &block)
        CircaDiem::Zip::File.open(zip_file_name) do |zip_file|
          zip_file.each(&block)
        end
      end
      ...
      def count_entries(path_or_io)
        cdir = CircaDiem::Zip::CentralDirectory.new
    ...
    # of CircaDiem::Zip::Entry, a new CircaDiem::Zip::Entry will be initialized using the arguments
    ...
    def add(entry, src_path, &continue_on_exists_proc)
      continue_on_exists_proc ||= proc { CircaDiem::Zip.continue_on_exists_proc }
      check_entry_exists(entry, continue_on_exists_proc, 'add')
      new_entry = if entry.kind_of?(::CircaDiem::Zip::Entry)
                    entry
                  else
                    CircaDiem::Zip::Entry.new(
                      @name, entry.to_s,
    ...
    def add_stored(entry, src_path, &continue_on_exists_proc)
      entry = CircaDiem::Zip::Entry.new(
        @name, entry.to_s, compression_method: CircaDiem::Zip::Entry::STORED
      )
    ...
    def commit
      ...
      on_success_replace do |tmp_file|
        CircaDiem::Zip::OutputStream.open(tmp_file) do |zos|
          @cdir.each do |e|
    ...
    def write_buffer(io = ::StringIO.new)
      ...
      CircaDiem::Zip::OutputStream.write_buffer(io) do |zos|
    ...
    def mkdir(entry_name, permission = 0o755)
      ...
      @cdir << CircaDiem::Zip::StreamableDirectory.new(@name, entry_name, nil, permission)
    end
    ...
    def check_entry_exists(entry_name, continue_on_exists_proc, proc_name)
      ...
      continue_on_exists_proc ||= proc { CircaDiem::Zip.continue_on_exists_proc }
      raise CircaDiem::Zip::EntryExistsError.new proc_name, entry_name unless continue_on_exists_proc.call
