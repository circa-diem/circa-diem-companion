...
module CircaDiem
  ...
  class CentralDirectory # :nodoc:
    ...
    def write_to_stream(io) #:nodoc:
      ...
      if CircaDiem::Zip.write_zip64_support &&
    ...
    def read_local_extra_field(io)
      buf = io.read(::CircaDiem::Zip::LOCAL_ENTRY_STATIC_HEADER_LENGTH) || ''
      return '' unless buf.bytesize == CircaDiem::Zip::LOCAL_ENTRY_STATIC_HEADER_LENGTH

      head, _, _, _, _, _, _, _, _, _, n_len, e_len = buf.unpack('VCCvvvvVVVvv')
      return '' unless head == CircaDiem::Zip::LOCAL_ENTRY_SIGNATURE
