...
Sketchup::require 'circadiem-companion/lib/zip/dirtyable'

module CircaDiem
  module Zip
  # CircaDiem::Zip::Entry represents an entry in a Zip archive.
  class Entry
    include Dirtyable

    STORED   = CircaDiem::Zip::COMPRESSION_METHOD_STORE
    DEFLATED = CircaDiem::Zip::COMPRESSION_METHOD_DEFLATE
    ...
    def set_default_vars_values
      ...
      @header_signature         = CircaDiem::Zip::CENTRAL_DIRECTORY_ENTRY_SIGNATURE
      ...
      @filepath        = nil
      @gp_flags        = 0
      if CircaDiem::Zip.unicode_names
        @gp_flags |= EFS
        @version = 63
      end
    ...
    def initialize(
      ...
      compression_level: CircaDiem::Zip.default_compression,
      time: CircaDiem::Zip::DOSTime.now, extra: CircaDiem::Zip::ExtraField.new
    )
      super()
      @name = name
      check_name(@name)
      ...
      @fstype = CircaDiem::Zip::RUNNING_ON_WINDOWS ? CircaDiem::Zip::FSTYPE_FAT : CircaDiem::Zip::FSTYPE_UNIX

      @zipfile            = zipfile
      @comment            = comment || ''
      @compression_method = compression_method || DEFLATED
      @compression_level  = compression_level || CircaDiem::Zip.default_compression
      @compressed_size    = compressed_size || 0
      @crc                = crc || 0
      @size               = size
      @time               = case time
                            when CircaDiem::Zip::DOSTime
                              time
                            when Time
                              CircaDiem::Zip::DOSTime.from_time(time)
                            else
                              CircaDiem::Zip::DOSTime.now
                            end
      @extra              =
    ...
    def extract(entry_path = @name, destination_directory: '.', &block)
      ...
      block ||= proc { CircaDiem::Zip.on_exists_proc }
    ...
    def read_local_entry(io) #:nodoc:all
      ...
      static_sized_fields_buf = io.read(::CircaDiem::Zip::LOCAL_ENTRY_STATIC_HEADER_LENGTH) || ''

      unless static_sized_fields_buf.bytesize == CircaDiem::Zip::LOCAL_ENTRY_STATIC_HEADER_LENGTH
      ...
      if CircaDiem::Zip.force_entry_names_encoding
        @name.force_encoding(::CircaDiem::Zip.force_entry_names_encoding)
      end
      ...
      if extra && extra.bytesize != @extra_length
        raise CircaDiem::Zip::Error, 'Truncated local zip entry header'
      end
    ...
    def pack_local_entry
      zip64 = @extra['Zip64']
      [::CircaDiem::Zip::LOCAL_ENTRY_SIGNATURE,
    ...
    def set_ftype_from_c_dir_entry
      @ftype = case @fstype
               when CircaDiem::Zip::FSTYPE_UNIX
                 @unix_perms = (@external_file_attributes >> 16) & 0o7777
                 case (@external_file_attributes >> 28)
                 when CircaDiem::Zip::FILE_TYPE_DIR
                   :directory
                 when CircaDiem::Zip::FILE_TYPE_FILE
                   :file
                 when CircaDiem::Zip::FILE_TYPE_SYMLINK
                   :symlink
                 else
    ...
    def check_c_dir_entry_static_header_length(buf)
      return unless buf.nil? || buf.bytesize != CircaDiem::Zip::CDIR_ENTRY_STATIC_HEADER_LENGTH

      raise Error, 'Premature end of file. Not enough data for zip cdir entry header'
    end

    def check_c_dir_entry_signature
      return if @header_signature == CircaDiem::Zip::CENTRAL_DIRECTORY_ENTRY_SIGNATURE
    ...
    def check_c_dir_entry_comment_size
      ...
      raise CircaDiem::Zip::Error, 'Truncated cdir zip entry header'
    end

    def read_extra_field(buf, local: false)
      if @extra.kind_of?(::CircaDiem::Zip::ExtraField)
        @extra.merge(buf, local: local) if buf
      else
        @extra = CircaDiem::Zip::ExtraField.new(buf, local: local)
      end
    end

    def read_c_dir_entry(io) #:nodoc:all
      @dirty = false # No changes at this point.
      static_sized_fields_buf = io.read(::CircaDiem::Zip::CDIR_ENTRY_STATIC_HEADER_LENGTH)
    ...
    def get_extra_attributes_from_path(path) # :nodoc:
      ...
      return if CircaDiem::Zip::RUNNING_ON_WINDOWS
    ...
    def write_c_dir_entry(io) #:nodoc:all
      ...
      when CircaDiem::Zip::FSTYPE_UNIX
        ft = case ftype
             when :file
               @unix_perms ||= 0o644
               CircaDiem::Zip::FILE_TYPE_FILE
             when :directory
               @unix_perms ||= 0o755
               CircaDiem::Zip::FILE_TYPE_DIR
             when :symlink
               @unix_perms ||= 0o755
               CircaDiem::Zip::FILE_TYPE_SYMLINK
             end
    ...
    def get_input_stream(&block)
      if ftype == :directory
        yield CircaDiem::Zip::NullInputStream if block
        CircaDiem::Zip::NullInputStream
      elsif @filepath
        case ftype
        when :file
        ...
        zis = CircaDiem::Zip::InputStream.new(@zipfile, offset: local_header_offset)
    ...
    def write_to_zip_output_stream(zip_output_stream) #:nodoc:all
      if ftype == :directory
        ...
        get_input_stream do |is|
          CircaDiem::Zip::IOExtras.copy_stream(zip_output_stream, is)
        end
      else
    ...
    def set_time(binary_dos_date, binary_dos_time)
      @time = CircaDiem::Zip::DOSTime.parse_binary_dos_format(binary_dos_date, binary_dos_time)
    rescue ArgumentError
      warn 'WARNING: invalid date/time in zip entry.' if CircaDiem::Zip.warn_invalid_date
    end

    def create_file(dest_path, _continue_on_exists_proc = proc { CircaDiem::Zip.continue_on_exists_proc })
      if ::File.exist?(dest_path) && !yield(self, dest_path)
        raise CircaDiem::Zip::DestinationExistsError, dest_path
      end
      ...
      ::File.open(dest_path, 'wb') do |os|
        get_input_stream do |is|
          ...
          warned = false
          buf = +''
          while (buf = is.sysread(::CircaDiem::Zip::Decompressor::CHUNK_SIZE, buf))
            os << buf
            bytes_written += buf.bytesize
            next unless bytes_written > size && !warned

            error = CircaDiem::Zip::EntrySizeError.new(self)
            raise error if CircaDiem::Zip.validate_entry_sizes
    ...
    def create_directory(dest_path)
      ...
      if ::File.exist?(dest_path)
        raise CircaDiem::Zip::DestinationExistsError, dest_path unless block_given? && yield(self, dest_path)
    ...
    def prep_local_zip64_extra
      return unless CircaDiem::Zip.write_zip64_support
