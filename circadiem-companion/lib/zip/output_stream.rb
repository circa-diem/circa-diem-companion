...
module CircaDiem
  ...
  class OutputStream
    extend Forwardable
    include CircaDiem::Zip::IOExtras::AbstractOutputStream
    ...
    def initialize(file_name, stream: false, encrypter: nil)
      ...
      @output_stream = if stream
                         iostream = CircaDiem::Zip::RUNNING_ON_WINDOWS ? @file_name : @file_name.dup
      ...
      @compressor = CircaDiem::Zip::NullCompressor.instance
      @encrypter = encrypter || CircaDiem::Zip::NullEncrypter.new
      @closed = false
      @current_entry = nil
    end
    ...
    def put_next_entry(
      ...
      compression_method = Entry::DEFLATED, level = CircaDiem::Zip.default_compression
    )
    ...
    def copy_raw_entry(entry)
      ...
      entry.get_raw_input_stream do |is|
        ...
        CircaDiem::Zip::Entry.read_local_entry(is)
    ...
    def get_compressor(entry)
      ...
      when Entry::DEFLATED
        CircaDiem::Zip::Deflater.new(@output_stream, entry.compression_level, @encrypter)
      when Entry::STORED
        CircaDiem::Zip::PassThruCompressor.new(@output_stream)
      else
        raise CircaDiem::Zip::CompressionMethodError, entry.compression_method
      end
    end
